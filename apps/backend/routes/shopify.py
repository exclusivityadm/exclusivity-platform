...
# Endpoints:
#   GET  /shopify/install?shop=<shop>&merchant_id=<uuid?>
#   GET  /shopify/callback
#   POST /shopify/webhooks
#   GET  /shopify/test?shop=<shop.myshopify.com>
# Requires env:
#   SHOPIFY_API_KEY
#   SHOPIFY_API_SECRET
#   SHOPIFY_SCOPES                 (e.g. "read_products,read_customers,read_orders,write_customers")
#   SHOPIFY_APP_URL                (backend base URL; fallback to RENDER_EXTERNAL_URL)
#   SHOPIFY_WEBHOOK_SECRET         (usually same as SHOPIFY_API_SECRET)
# Optional:
#   FRONTEND_URL                   (where to redirect after callback; else JSON)
#
# Supabase tables (create via schema.sql):
#   shopify_shops(shop text pk, access_token text, scope text, merchant_id uuid, installed_at timestamptz)
#
# Notes:
# - We verify OAuth callback HMAC (query) and Webhook HMAC (header).
# - We accept bare shop names and append ".myshopify.com" if needed.
# - Minimal webhook handlers are included; extend as necessary.
# ============================================================
...
def _verify_hmac_from_query(params: Dict[str, str]) -> bool:
    """
    Verify Shopify HMAC in OAuth callback query using API secret.
    HMAC is computed over all query params except 'hmac' sorted lexicographically.
    """
...
def _ensure_webhook(shop: str, access_token: str, topic: str, address: str):
    """
    Idempotently ensure a webhook exists for topic/address.
    """
    ...
    try:
        ...
        # Don't break install if webhook creation fails; handle later.
        pass
...
def _get_shop_token(shop: str) -> Optional[str]:
    sb = get_supabase()
    if not sb:
        return None
...
def install(shop: str, merchant_id: Optional[str] = None):
    """
    Start OAuth flow. Example:
      GET /shopify/install?shop=mybrand (or mybrand.myshopify.com)&merchant_id=<uuid?>
    """
    _require_env()
...
def callback(request: Request):
    """
    OAuth callback: verify HMAC, exchange code for token, store token, register webhooks.
    """
    _require_env()
    ...
    # Register webhooks (non-fatal if fails)
    try:
    ...
    # Redirect to frontend admin or return JSON
    if FRONTEND_URL:
...
async def webhooks(request: Request):
    """
    Verify webhook HMAC and do minimal handling.
    - customers/create: upsert into customers table if merchant link is known.
    - orders/paid: (placeholder) — wire points accrual here later.
    """
    ...
    if sb and topic == "customers/create":
        # Link to merchant if this shop was installed with merchant_id
        try:
            ...
            # swallow to avoid webhook retry storms; inspect logs if needed
            pass

    if sb and topic == "orders/paid":
        # Placeholder: add points accrual based on order total here (next drop).
        # Keep a minimal no-op to return 200 quickly.
        pass
...
def test(shop: str):
    """
    Smoke test: calls /admin/api/<ver>/shop.json using stored token.
      GET /shopify/test?shop=mybrand.myshopify.com
    """
    shop_norm = _normalize_shop(shop)
    token = _get_shop_token(shop_norm)
    if not token:
